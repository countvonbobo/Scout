...
const WINDOWS_PRIVATE_DIRECTORY_SCRIPT = [
  ...
  "$descriptorMatch=[regex]::Match($savedAcl,'(?m)^D:(?<flags>[A-Z]*)(?<aces>\\(.*\\))\\s*$')",
  ...
  "$genericAce='(A;OICI;GA;;;'+$identity.User.Value+')'",
  "$reorderedAce='(A;CIOI;FA;;;'+$identity.User.Value+')'",
  'if($aces -ne $expectedAce) {',
  '  if($aces -eq $genericAce) { exit 44 }',
  '  if($aces -eq $reorderedAce) { exit 45 }',
  "  $currentOnly='^(?:\\(A;[A-Z]*;[A-Z0-9x]*;;;'+[regex]::Escape($identity.User.Value)+'\\))+$'",
  '  if($aces -match $currentOnly) {',
  "    if(([regex]::Matches($aces,'\\(A;')).Count -gt 1) { exit 46 }",
  '    exit 47',
  '  }',
  '  exit 42',
  '}',
