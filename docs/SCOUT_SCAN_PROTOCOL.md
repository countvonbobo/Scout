...
1. **Normalises and deduplicates every returned record.** Source order does not affect the canonical vacancy set. Source/portal errors are counted separately from returned records that fail normalisation, so neither kind of failure can create a negative observation count.
