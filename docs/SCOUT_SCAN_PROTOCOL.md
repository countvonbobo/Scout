...
2. **Normalises and deduplicates every returned record.** Source order does not affect the canonical vacancy set. A URL-less vacancy ID is anchored to a bounded canonical provider reference, so distinct same-title openings remain distinct even when returned in different scans; cross-source copies still reconcile through semantic and reference evidence. Source/portal errors are counted separately from returned records that fail normalisation, so neither kind of failure can create a negative observation count. A record and its canonical vacancy retain every matching lane ID.
