...
test('URL-less canonical identities are stable, collision-free and source-order independent', () => {
  ...
  assert.deepEqual(forward, reverse);
  assert.notEqual(forward.vacancies[0].vacancyId, firstAloneId);
  ...
  assert.notEqual(firstWithEarlier[0].vacancyId, firstAloneId);
  ...
  assert.notEqual(firstWithGreater[0].vacancyId, firstAloneId);
