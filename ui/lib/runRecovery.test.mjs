...
function appendStage(run, lease, stageId, stableIds = [`${stageId}-vacancy`]) {
  const artifact = commitRunArtifact(
    run,
    { id: `${stageId}-artifact`, schemaVersion: 1 },
  ...
  appendRunEvent(run, {
    ...
    stageId,
    idempotencyKey: `${stageId}-completed-v1`,
    payload: {
      schemaVersion: 1,
...
test('keeps partial and abandoned candidates immutable while selecting an older compatible run', () => {
  ...
  assert.deepEqual(result.skipped, [
    { runId: 'run-partial', reasons: ['terminal-partial'] },
    { runId: 'run-abandoned', reasons: ['terminal-abandoned'] },
  ]);
...
test('validates the journal before rebuilding the manifest and journals explicit substitution provenance', () => {
  ...
  appendStage(run, lease, 'rank');
  appendStage(run, lease, 'assess');
  validateManifestAgreement(run, lease);
  ...
  const recovered = recoverRun(root, runId, lease);
  ...
  assert.deepEqual(recovered.reusableStages.map((stage) => stage.stageId), ['collect', 'rank']);
...
test('fails closed without changing a damaged run and writes a fixed external recovery failure record', () => {
  ...
  lease = acquireScanLease(root, currentLeaseOwner(), operation(runId));
  assert.throws(() => recoverRun(root, runId, lease), /journal.*damaged/i);
  ...
  assert.deepEqual(
    ...
    ['eventId', 'fencingGeneration', 'leaseId', 'reason', 'recordedAt', 'runId', 'schemaVersion'].sort(),
  );
...
test('manifest validation covers artifact references in recovery decisions', () => {
  ...
  assert.throws(() => validateManifestAgreement(run, lease), ArtifactIntegrityError);
});
